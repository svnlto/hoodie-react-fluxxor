(function(root, factory) {
  ...
  Backbone.sync = function(method, modelOrCollection, options) {
    ...
    case 'read':
      ...
      break;
    case 'create':
      promise = Backbone.hoodie.store.add(type, attributes, options).done(function(attributes) {
        modelOrCollection.set(attributes);
      });
      break;
    case 'update':
      promise = Backbone.hoodie.store.updateOrAdd(type, id, attributes).done(function(attributes) {
        modelOrCollection.set(attributes);
      });
      break;
    case 'delete':
